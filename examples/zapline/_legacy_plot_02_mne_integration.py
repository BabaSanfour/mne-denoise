...
from mne_denoise import dss_zapline, zapline_plus, compute_psd_reduction
...
if HAS_MNE:
    ...
    ax.semilogy(freqs, psd_removed_mean, 'r-', linewidth=1.5, alpha=0.7, label='Removed')
    ax.set_xlabel('Frequency (Hz)')
    ax.set_ylabel('PSD (T/m)²/Hz')
...
if HAS_MNE:
    ...
    if len(raw_eeg.ch_names) > 0:
        ...
        result_eeg = dss_zapline(
            data_eeg, line_freq=line_freq, sfreq=sfreq,
            n_remove='auto',
        )
