...
def generate_nonstationary_data(
    ...
    return data, times, {
        ...
        'topo_a': topo_a, 'topo_b': topo_b, 'topo_c': topo_c
    }
...
segments = segment_data(
    data, sfreq, target_freq,
    min_chunk_len=PAPER_PARAMS['minChunkLength']
)
print(f"Number of segments: {len(segments)}")
for i, (start, end) in enumerate(segments):
    print(f"  Segment {i+1}: {start/sfreq:.1f}s - {end/sfreq:.1f}s ({(end-start)/sfreq:.1f}s)")
...
im = ax3.pcolormesh(t_spec, f_spec, 10*np.log10(Sxx+1e-20), 
                     shading='gouraud', cmap='viridis', vmin=-30, vmax=10)
ax3.set_ylim(45, 55)
...
im = ax4.pcolormesh(t_spec, f_spec, 10*np.log10(Sxx_clean+1e-20), 
                     shading='gouraud', cmap='viridis', vmin=-30, vmax=10)
ax4.set_ylim(45, 55)
...
im = ax5.pcolormesh(t_spec, f_spec, 10*np.log10(Sxx_rem+1e-20), 
                     shading='gouraud', cmap='viridis', vmin=-30, vmax=10)
ax5.set_ylim(45, 55)
...
# Per segment
ratio_o1, ratio_c1 = compute_noise_ratio(data[:, :t1], data_clean[:, :t1], sfreq, sim_info['freq1'])
ratio_o2, ratio_c2 = compute_noise_ratio(data[:, t1:t2], data_clean[:, t1:t2], sfreq, sim_info['freq2'])
ratio_o3, ratio_c3 = compute_noise_ratio(data[:, t2:], data_clean[:, t2:], sfreq, sim_info['freq3'])
