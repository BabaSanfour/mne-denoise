...
from mne_denoise import zapline_plus, dss_zapline, ZapLinePlusResult
...
print(f"Non-stationary noise with:")
print(f"  - 50 Hz: moderate in first half, strong in second half")
print(f"  - 21 Hz: only in second half (simulating VR artifact)")
...
mask = f_spec < 100
im = ax.pcolormesh(t_spec, f_spec[mask], 10*np.log10(Sxx[mask]), 
...
if result_plus.chunk_results:
    ...
    for i, r in enumerate(result_plus.chunk_results):
        ...
        print(f"Chunk {i+1}: {time_start:.1f}-{time_end:.1f}s | "
    ...
    chunk_times = [(r['start'] + r['end']) / 2 / sfreq for r in result_plus.chunk_results]
    ...
    ax = axes[1]
    ax.bar(chunk_times, chunk_removed, width=duration/len(chunk_times)*0.8, color='green', alpha=0.7)
    ax.set_xlabel('Time (s)')
