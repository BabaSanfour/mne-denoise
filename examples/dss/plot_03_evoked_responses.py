...
from mne_denoise.dss import DSS, TrialAverageBias, LinearDenoiser
...
epochs_sim = mne.EpochsArray(epochs_data, info_sim, events=events_sim, tmin=-0.5, verbose=False)
...
plt.plot(times, evoked_source / np.max(np.abs(evoked_source)), 'r-', label="Recovered (DSS Comp 0)")
plt.plot(times, epochs_sim.average().data[0] / np.max(np.abs(epochs_sim.average().data[0])), 'g-', alpha=0.3, label="Channel Average (Noisy)")
...
dss_std = DSS(n_components=10, bias=TrialAverageBias(), cov_method='empirical', cov_kws={'n_jobs': 1})
...
epochs_clean = mne.EpochsArray(clean_data, epochs_meg.info, tmin=epochs_meg.tmin, events=epochs_meg.events, event_id=epochs_meg.event_id)

# Compare Global Field Power
plot_evoked_comparison(epochs_meg, epochs_clean, labels=('Original', f'Denoised (Top {keep_n})'), show=False)
...
print(f"SNR Improvement: {snr_in:.2f} -> {snr_out:.2f} ({snr_out/snr_in:.1f}x improvement)")
...
# 2. Fit DSS
dss_diff = DSS(n_components=10, bias=custom_bias, cov_method='empirical', cov_kws={'n_jobs': 1})
