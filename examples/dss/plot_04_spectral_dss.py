...
src = sources[0, :500]
if np.corrcoef(src, alpha[:500])[0, 1] < 0: src *= -1
plt.plot(times[:500], src, 'r')
plt.plot(times[:500], alpha[:500] * (np.max(np.abs(src))/np.max(np.abs(alpha))), 'k--', alpha=0.5, label='Truth')
...
plot_narrowband_scan(freqs_meg, eigs_meg, peak_freq=freqs_meg[np.argmax(eigs_meg)], show=False)
...
dss_meg = narrowband_dss(sfreq=raw.info['sfreq'], freq=peak_freq, bandwidth=3.0, n_components=5)
dss_meg.fit(raw)
...
plot_spectral_psd_comparison(raw, sources_meg, raw.info['sfreq'], peak_freq=peak_freq, show=False)
...
raw_eeg = read_raw_edf(eegbci.load_data(subjects=[1], runs=[1])[0], preload=True, verbose=False)
...
print(f"EEG Data: {len(raw_eeg.ch_names)} channels, {raw_eeg.times[-1]:.1f}s (eyes closed)")
...
dss_eeg = narrowband_dss(sfreq=raw_eeg.info['sfreq'], freq=peak_freq_eeg, bandwidth=3.0, n_components=5)
dss_eeg.fit(raw_eeg)
...
# 2. PSD Comparison: Before vs After
plot_spectral_psd_comparison(raw_eeg, sources_eeg, raw_eeg.info['sfreq'], peak_freq=peak_freq_eeg, show=False)
