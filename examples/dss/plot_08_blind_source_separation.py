...
from mne_denoise.dss import IterativeDSS, TanhMaskDenoiser, KurtosisDenoiser, beta_tanh
...
print(f"  Newton Iterations:   {iters_newton:.0f} (Speedup: {iters_grad/iters_newton:.1f}x)")
...
raw = mne.io.read_raw_fif(raw_fname, verbose=False) # list_url=[] prevents download print spam usually
...
eog_raw = mne.io.RawArray(eog_ch[None, :], mne.create_info(['EOG'], raw.info['sfreq'], 'eog'))
comp_raw = mne.io.RawArray(sources[blink_idx:blink_idx+1], 
                           mne.create_info(1, raw.info['sfreq'], 'misc'))
