...
# 1. Create EOG Epochs
eog_epochs = create_eog_epochs(raw, ch_name='EOG 061', baseline=(-0.5, -0.2), 
                             tmin=-0.5, tmax=0.5, verbose=False)
...
plt.plot(times_plot, eog_data[:n_samples_plot] * scaler_eog, label='EOG Channel (Norm)', color='tab:orange', alpha=0.7)
plt.plot(times_plot, blink_source[:n_samples_plot] * scaler_dss * sign, label='DSS Comp 0 (Sign-Matched)', color='tab:blue', alpha=0.7)
...
eog_epochs_clean = mne.Epochs(raw_clean_eog, eog_epochs.events, tmin=-0.5, tmax=0.5, 
                            baseline=(-0.5, -0.2), verbose=False)
...
plot_evoked_comparison(eog_epochs, eog_epochs_clean, show=False, labels=('Original', 'Cleaned'))
...
ecg_epochs = create_ecg_epochs(raw, ch_name=None, tmin=-0.1, tmax=0.1, 
                             baseline=(None, 0), verbose=False)
ecg_epochs.pick_types(meg='grad', eeg=False, eog=False, ecg=False)
print(f"Found {len(ecg_epochs)} heartbeats. Using {len(ecg_epochs.ch_names)} MEG channels.")
...
# Verification
ecg_epochs_clean = mne.Epochs(raw_clean_ecg, ecg_epochs.events, tmin=-0.1, tmax=0.1,
                            baseline=(None, 0), verbose=False)

# Expectation 1: The QRS spike should be suppressed.
plot_evoked_comparison(ecg_epochs, ecg_epochs_clean, show=False, labels=('Original', 'Cleaned'))
