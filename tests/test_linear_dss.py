...
def test_compute_dss_known_signal():
    ...
    alignment = np.abs(np.dot(top_filter / np.linalg.norm(top_filter), expected_direction))
...
def test_dss_inverse_transform_mne_format_3d():
    ...
    sources_mne_format = np.transpose(sources, (2, 0, 1))  # (n_epochs, n_comps, n_times)
...
def test_dss_normalization_with_different_scales():
    ...
    assert dss_norm.channel_norms_[0] < dss_norm.channel_norms_[1] < dss_norm.channel_norms_[2]
...
def test_dss_cov_method_options():
    ...
    dss = DSS(n_components=2, bias=lambda x: x, cov_method='shrinkage', cov_kws={'shrinkage': 0.1})
    dss.fit(data)
    ...
    dss_mne = DSS(n_components=2, bias=lambda x: x, cov_method='auto', cov_kws={'return_estimators': False})
    dss_mne.fit(raw)
