...
def _iterative_outlier_removal(scores: np.ndarray, sigma: float = 3.0) -> int:
    """Iteratively detect outliers using mean + sigma threshold.
    ...
    *,
    ...
    if noisefreqs == 'line':
        ...
        noisefreqs = _find_noise_frequencies(
            data, sfreq, minfreq, maxfreq,
            detection_winsize, coarse_freq_detect_power_diff
        )
        if verbose:
    ...
    for freq in noisefreqs:
        ...
        chunks = _determine_chunks(
            cleaned, sfreq, freq,
            chunk_length, min_chunk_length, detection_winsize
        )
        ...
        for max_iter in range(8):  # Max 8 adaptation iterations
            ...
            assessment = _assess_cleaning(
                data, cleaned, sfreq, freq, detection_winsize
            )
            ...
            if assessment['too_strong'] and current_sigma < maxsigma:
                ...
                if verbose:
                    print(f"    Cleaning too strong, relaxing (sigma={current_sigma:.2f})")
                continue
            ...
            if assessment['too_weak'] and current_sigma > minsigma:
                ...
                current_fixed += 1
                if verbose:
                    print(f"    Cleaning too weak, strengthening (sigma={current_sigma:.2f})")
                continue
    ...
    if verbose:
        ...
        for freq in noisefreqs:
            ...
            if ratio_key in analytics:
                print(f"  Noise/surroundings ratio at {freq} Hz: {analytics[ratio_key]:.2f}")
...
def _find_noise_frequencies(
    ...
    while search_start < maxfreq:
        ...
        mask = (freqs >= search_start) & (freqs <= min(search_start + detection_winsize, maxfreq))
        if not np.any(mask):
            break
...
def _determine_chunks(
    ...
    peaks, _ = signal.find_peaks(distances, prominence=prominence, distance=min_distance)
...
def _assess_cleaning(
    ...
    mask = (freqs >= freq - detection_winsize / 2) & (freqs <= freq + detection_winsize / 2)
    if not np.any(mask):
    ...
    lower_quantile = np.mean([
        np.quantile(window_psd[:third], 0.05),
        np.quantile(window_psd[-third:], 0.05)
    ])
...
def _compute_analytics(
    ...
    proportion_removed = 1 - 10 ** ((np.mean(psd_clean_log) - np.mean(psd_orig_log)) / 10)
    ...
    for freq in noisefreqs:
        ...
        surround_mask = (
            ((freqs >= freq - 3) & (freqs <= freq - 1)) |
            ((freqs >= freq + 1) & (freqs <= freq + 3))
        )
        
        if np.any(freq_mask) and np.any(surround_mask):
            ratio_orig = np.mean(psd_orig[:, freq_mask]) / np.mean(psd_orig[:, surround_mask])
            ratio_clean = np.mean(psd_clean[:, freq_mask]) / np.mean(psd_clean[:, surround_mask])
